...
class ConversationalRAG:
    ...
    def save_iteration(self, conversation, iteration, full_history):
        ...
        print(f"\n[SAVE] Storing CR_ID={conversation}, Iteration={iteration}")
    ...
    def retrieve_full_history(self, cr_id):
        """Retrieve all stored iterations for a given conversation ID (CR_ID)."""
        print(f"[RETRIEVE] Querying stored messages for CR_ID={cr_id}.")
        ...
        query_result = self.index.query(
            ...
            filter={"cr_id": {"$eq": str(cr_id)}}
        )

        print(f"[RAW QUERY RESULTS] {query_result}")

        # Extract stored messages only for the matching CR_ID
        retrieved_messages = [
